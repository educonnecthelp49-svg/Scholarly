...
class User(db.Model):
    ...
    points = db.Column(db.Integer, default=0, nullable=False)  # Student points system - default 0
...
class File(db.Model):
    """Model for storing files in base64 format in database"""
    id = db.Column(db.Integer, primary_key=True)
    filename = db.Column(db.String(255), nullable=False)
    file_data = db.Column(db.Text, nullable=False)  # Base64 encoded file data
    file_size = db.Column(db.Integer, nullable=False)  # Size in bytes
    mime_type = db.Column(db.String(100), nullable=False)
    file_type = db.Column(db.String(50), nullable=False)  # 'classwork', 'homework', 'circular', 'profile_photo', etc.
...
class Classwork(db.Model):
    ...
    file = db.relationship('File', backref='classworks')
    ...
    def file_size_mb(self):
        """Get file size in MB"""
        if self.file and self.file.file_size:
            return round(self.file.file_size / (1024 * 1024), 2)
        return 0
...
class Homework(db.Model):
    ...
    # Optional attachment
    attachment_file_id = db.Column(db.Integer, db.ForeignKey('file.id'))
    
    teacher = db.relationship('User', backref=db.backref('homework_assignments', lazy='dynamic'))
    attachment_file = db.relationship('File', foreign_keys=[attachment_file_id])
    ...
    def attachment_size(self):
        ...
        if self.attachment_file:
            return self.attachment_file.file_size
        return 0
...
class Announcement(db.Model):
    ...
    photo_file_id = db.Column(db.Integer, db.ForeignKey('file.id'))
    ...
    def photo_size_mb(self):
        ...
        if self.photo_file and self.photo_file.file_size:
            return round(self.photo_file.file_size / (1024 * 1024), 2)
        return 0
...
Index('idx_file_uploaded_by', File.uploaded_by_id)
Index('idx_file_file_type', File.file_type)
Index('idx_file_upload_date', File.upload_date)
