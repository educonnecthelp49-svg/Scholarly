...
from models import (User, Post, Comment, Like, DirectMessage, Group, GroupMembership, 
                   ...
                   ParentChild, DailyPostingActivity, CalendarEvent, Announcement, Report, File, get_ist_now)
...
def admin_dashboard():
    ...
    # Paginated students query - filter for students only (not admin, teacher, or parent)
    page = request.args.get('page', 1, type=int)
    per_page = 50  # Limit to 50 students per page
    
    students_pagination = User.query.filter_by(is_admin=False, is_teacher=False, is_parent=False)\
...
def send_message(username):
    ...
    emit_notification_update(receiver.id)
...
def create_homework():
    ...
    if 'attachment' in request.files:
        ...
        if file.filename != '':
            ...
            db.session.add(file_record)
            db.session.flush()  # Get the file_id
            attachment_file_id = file_record.id
...
def download_classwork(classwork_id):
    ...
    if not classwork.file:
        flash('File not found.', 'error')
        return redirect(url_for('teacher_classwork'))
    
    # Decode base64 data
    file_data = base64.b64decode(classwork.file.file_data)
    
    from flask import send_file
    from io import BytesIO
    return send_file(BytesIO(file_data), 
                     as_attachment=True, 
                     download_name=classwork.file.filename,
                     mimetype=classwork.file.mime_type)

@app.route('/download/homework/<int:homework_id>')
def download_homework_attachment(homework_id):
    """Download homework attachment"""
    if 'user_id' not in session:
        flash('Please log in to download homework attachments.', 'error')
        return redirect(url_for('login'))
    
    current_user = User.query.get(session['user_id'])
    homework = Homework.query.get_or_404(homework_id)
    
    # Check if user has permission to access this homework
    if current_user.is_student() and not homework.is_visible_to_student(current_user):
        flash('You do not have permission to access this homework.', 'error')
        return redirect(url_for('view_homework'))
    elif current_user.is_teacher and homework.teacher_id != current_user.id and not current_user.is_admin:
        flash('You do not have permission to access this homework.', 'error')
        return redirect(url_for('teacher_homework'))
    
    if not homework.attachment_file:
    ...
    # Decode base64 data
    file_data = base64.b64decode(homework.attachment_file.file_data)
    
    from flask import send_file
    from io import BytesIO
    return send_file(BytesIO(file_data), 
                     as_attachment=True,
                     download_name=homework.attachment_file.filename,
                     mimetype=homework.attachment_file.mime_type)
...
def download_circular_attachment(circular_id):
    ...
    if not circular.attachment_file:
        flash('Attachment not found.', 'error')
        return redirect(url_for('view_circulars'))
    
    # Decode base64 data
    file_data = base64.b64decode(circular.attachment_file.file_data)
    
    from flask import send_file
    from io import BytesIO
    return send_file(BytesIO(file_data), 
                     as_attachment=True,
                     download_name=circular.attachment_file.filename,
                     mimetype=circular.attachment_file.mime_type)
...
def delete_announcement(announcement_id):
    ...
    # The file will be deleted automatically due to cascade delete on the relationship
...
def announcement_photo(announcement_id):
    ...
    if not announcement.photo_file:
        return redirect(url_for('static', filename='images/placeholder.jpg')), 404
    
    # Decode base64 data
    file_data = base64.b64decode(announcement.photo_file.file_data)
    
    from flask import send_file
    from io import BytesIO
    return send_file(BytesIO(file_data), mimetype=announcement.photo_file.mime_type)
...
def review_report(report_id):
    ...
    # Debug logging
    print(f"DEBUG: review_report called with report_id: {report_id}")
    print(f"DEBUG: session user_id: {session.get('user_id')}")
    print(f"DEBUG: session is_admin: {session.get('is_admin')}")
    ...
    print(f"DEBUG: Found report: {report.id}, status: {report.status}")
    
    if report.status != 'pending':
        print("DEBUG: Report already reviewed")
    ...
    print(f"DEBUG: admin_notes: {admin_notes}")
    print(f"DEBUG: points_adjustment: {points_adjustment}")
...
def emit_notification_update(user_id):
